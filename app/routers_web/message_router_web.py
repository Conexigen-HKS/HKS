...
async def get_messages_page(
    ...
    # Subquery to get the latest message per user pair
    subquery = (
        db.query(
            func.greatest(Message.author_id, Message.receiver_id).label("user_pair"),
            func.least(Message.author_id, Message.receiver_id).label("pair_partner"),
            func.max(Message.created_at).label("latest_time"),
        )
        .filter((Message.author_id == current_user.id) | (Message.receiver_id == current_user.id))
        .group_by(func.greatest(Message.author_id, Message.receiver_id), func.least(Message.author_id, Message.receiver_id))
        .subquery()
    )

    # Join the main messages table with the subquery to get the latest message for each pair
    conversations = (
        db.query(Message)
        .join(
            subquery,
