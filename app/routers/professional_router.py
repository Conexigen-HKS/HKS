...
@professional_router.post("/job-applications", response_model=JobApplicationResponse)
def create_application(
    data: JobApplicationCreate,
    current_user: User = Depends(get_current_user),
    db: Session = Depends(get_db)
):
    return create_job_application(db, current_user.id, data)
...
@professional_router.put("/job-applications/{job_application_id}", response_model=JobApplicationResponse)
def edit_application(
    ...
    return edit_job_application(db, job_application_id, current_user.id, data)
